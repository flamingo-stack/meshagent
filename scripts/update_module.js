...
function updateModule(moduleName) {
    ...
    if (moduleName === 'service-manager') {
        ...
        // Smaller module with duk_peval_string_noresult format
        const regex = /duk_peval_string_noresult\(ctx, "addCompressedModule\('agent-installer', Buffer\.from\('[^']+', 'base64'\), '[^']+'\);"\);/;
        const match = cContent.match(regex);
        if (!match) {
            console.error('Could not find agent-installer in C file');
            process.exit(1);
        }
        const newLine = `duk_peval_string_noresult(ctx, "addCompressedModule('agent-installer', Buffer.from('${base64Data}', 'base64'), '${timestamp}');");`;
        cContent = cContent.replace(regex, newLine);
    } else {
