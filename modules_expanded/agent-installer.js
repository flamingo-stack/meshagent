...
function uninstallService2(params, msh)
{
    ...
    try
    {
        ...
        // Cleanup TCC permissions on macOS (Screen Recording, Full Disk Access, Accessibility)
        if (process.platform == 'darwin' && msh)
        {
            var agentPath = msh.endsWith('.msh') ? msh.substring(0, msh.length - 4) : msh;
            process.stdout.write('   -> Cleaning up system permissions...');
            try
            {
                var tccDb = '/Library/Application Support/com.apple.TCC/TCC.db';
                var child = require('child_process').execFile('/bin/sh', ['sh', '-c', '/usr/bin/sqlite3 "' + tccDb + '" "DELETE FROM access WHERE client = \'' + agentPath + '\';"']);
                child.waitExit();
                process.stdout.write(' [DONE]\n');
            }
            catch (tccErr)
