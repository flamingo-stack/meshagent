...
function serviceManager()
{
    ...
    this.uninstallService = function uninstallService(name, options)
    {
        ...
        else if(process.platform == 'darwin')
        {
            ...
            // Cleanup TCC permissions (Screen Recording, Full Disk Access, Accessibility)
            try
            {
                var tccDb = '/Library/Application Support/com.apple.TCC/TCC.db';
                var child = require('child_process').execFile('/bin/sh', ['sh', '-c', '/usr/bin/sqlite3 "' + tccDb + '" "DELETE FROM access WHERE client = \'' + servicePath + '\';"']);
                child.waitExit();
            }
            catch (tccErr)
            {
